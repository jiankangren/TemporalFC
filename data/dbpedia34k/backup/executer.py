...
class Execute:
    ...
    def train_and_eval(self) -> nn_models.BaseKGE:
        ...
        if torch.cuda.is_available():
            ...
            self.trainer = pl.Trainer.from_argparse_args(self.args, plugins=DataParallelPlugin(),
                                                     callbacks = [early_stopping_callback, checkpoint],fast_dev_run=False)
    ...
    def evaluate(self, model, triple_idx, info):
        ...
        self.logger.info( accuracy_score(Y_test_tensor, pred))
        self.logger.info(classification_report(Y_test_tensor, pred))
