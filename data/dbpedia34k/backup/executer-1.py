...
class Execute:
    ...
    def train_and_eval(self) -> nn_models.BaseKGE:
        ...
        if torch.cuda.is_available():
            ...
            self.trainer = pl.Trainer.from_argparse_args(self.args, plugins=DataParallelPlugin(),
                                                     callbacks = [early_stopping_callback, checkpoint],fast_dev_run=True)
    ...
    def training(self):
        ...
        dataset = StandardDataModule(train_set_idx=self.args.dataset.idx_train_set,
                                     ...
                                     entities_idx=self.args.dataset.idx_ent_dict,
                                     relations_idx=self.args.dataset.idx_rel_dict,
                                     times_idx=self.args.dataset.idx_time_dict,
    ...
    def evaluate(self, model, triple_idx, info):
        ...
        if info.__contains__("Test"):
            prob = model.forward_triples(idx_s, idx_p, idx_o, t_idx, v_data,type="test").flatten()
        else:
            prob = model.forward_triples(idx_s, idx_p, idx_o, t_idx, v_data).flatten()
        pred = (prob > 0.5).float()
